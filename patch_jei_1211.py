...
try:
    with urllib.request.urlopen(req) as response:
        ...
        # Unzip
        extract_dir = os.path.join(tempfile.gettempdir(), 'jei_extract_1211')
        ...
        if os.path.exists(toml_path):
            ...
            import re
            # Re-map dependencies to accept 1.21.2 and modern neoforge 21.2.1-beta
            toml_content = re.sub(r'versionRange=\x22\[21.*?\x22', 'versionRange=\x22[21.2.1-beta,)\x22', toml_content)
            toml_content = re.sub(r'versionRange=\x22\[1\.21.*?\x22', 'versionRange=\x22[1.21.2,1.22)\x22', toml_content)
            
            with open(toml_path, 'w', encoding='utf-8') as f:
                f.write(toml_content)
                
        new_filename = original_filename.replace('1.21.1', '1.21.2-patched')
        ...
        # Clean up old patched versions
        old_patch = os.path.join(target_dir, 'jei-1.21.2-patched-neoforge-20.0.0.4.jar')
        if os.path.exists(old_patch):
            os.remove(old_patch)
